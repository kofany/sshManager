...
func (v *editView) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.MouseMsg:
        // Handle mouse events for edit view
        switch msg.Type {
        case tea.MouseLeft:
            ...
            if v.mode == modeNormal && v.editingHost {
                ...
                for _, pos := range fieldPositions {
                    if msg.Y == pos.y && msg.X >= pos.x && msg.X <= pos.x+pos.width {
                        v.activeField = pos.fieldIndex
                        v.inputs[pos.fieldIndex].Focus()
                        // Blur other fields
                        for i := range v.inputs {
                            if i != pos.fieldIndex {
                                v.inputs[i].Blur()
                ...
                if msg.Y >= listStartY {
                    if v.authTypePasswords {
                        ...
                        } else if msg.Y >= listStartY && msg.Y < passwordsEndY {
        ...
        case tea.MouseWheelDown:
            ...
            if v.mode == modeSelectPassword {
                if v.authTypePasswords {
                    if v.selectedPasswordIndex < len(v.passwordList)-1 {
                        v.selectedPasswordIndex++
                    }
                } else {
                    if v.selectedPasswordIndex < len(v.keys)-1 {
                        v.selectedPasswordIndex++
                    }
                }
            } else if v.mode == modePasswordList {
                if v.selectedItemIndex < len(v.passwords)-1 {
                    v.selectedItemIndex++
                }
            } else if v.mode == modeKeyList {
                if v.selectedItemIndex < len(v.keys)-1 {
                    v.selectedItemIndex++
                }
            }
        }
