...
func (m *initialPromptModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.MouseMsg:
        // Handle mouse events for initial password prompt
        switch msg.Type {
        case tea.MouseLeft:
            // Handle clicks on buttons (simplified positioning)
            // Submit button area (bottom of screen)
            if msg.Y >= m.height-3 && msg.X >= m.width/2-10 && msg.X <= m.width/2+10 {
                if len(m.password) == 0 {
...
func (m *ApiKeyPromptModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.MouseMsg:
        // Handle mouse events for API key prompt
        switch msg.Type {
        case tea.MouseLeft:
            // Handle clicks on buttons (simplified positioning)
            // Submit button area (bottom of screen)
            if msg.Y >= m.height-3 && msg.X >= m.width/2-10 && msg.X <= m.width/2+10 {
                ...
                // Check basic API key validation
                if len(apiKey) < 32 {
            ...
            // Local mode button area (left side)
            if msg.Y >= m.height-3 && msg.X >= 5 && msg.X <= 25 {
                return m, tea.Sequence(
                    tea.ClearScreen,
                    func() tea.Msg {
                        return tea.WindowSizeMsg{
            ...
            inputY := m.height/2 + 2
            if msg.Y == inputY && msg.X >= m.width/2-20 && msg.X <= m.width/2+20 {
                m.input.Focus()
                return m, nil
            }
        }
