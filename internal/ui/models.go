...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.MouseMsg:
        ...
        switch msg.Type {
        case tea.MouseLeft:
            // Handle left click - will be handled by individual views
        case tea.MouseWheelUp:
            // Handle scroll up
            if m.activeView == ViewHostList {
                var cmd tea.Cmd
                m.hostList, cmd = m.hostList.Update(msg)
                return m, cmd
            } else if m.activeView == ViewPasswordList {
                var cmd tea.Cmd
                m.passwordList, cmd = m.passwordList.Update(msg)
                return m, cmd
            }
...
func initializeList(title string) list.Model {
    ...
    delegate.Styles = list.NewDefaultDelegate().Styles
    
    l := list.New([]list.Item{}, delegate, 0, 0)
    l.Title = title
    l.SetShowHelp(true)
    l.SetFilteringEnabled(false)
    l.DisableQuitKeybindings() // Allow us to handle mouse events properly
    return l
}
